(function(){
  var Client, Connection, ResponseHandler, make_command_method;
  ...
  Client.prototype.DEFAULT_PORT = 11300;
  Client.prototype.LOWEST_PRIORITY = 4294967295;
  Client.prototype.connect = function (server, callback) {
    ...
    addr = addr || Client.DEFAULT_ADDR;
    port = Number(port) || Client.DEFAULT_PORT;
    ...
    return stream.on('close', function(has_error) {
      return me.emit('close', has_error);
    });
  };
  make_command_method = function(command_name, expected_response, sends_data) {
    return function() {
      ...
      return this.handlers.push([new ResponseHandler(command_name, expected_response), callback]);
    };
  };
  ...
  Connection.prototype.try_handling_response = function() {
    var _a, commandName, callback, handler;
    _a = this.handlers[0];
    handler = _a[0];
    callback = _a[1];
  ...
  Connection.prototype.kick_job = make_command_method('kick-job', 'KICKED');
  ...
  ResponseHandler = function(command_name, success_code) {
  ...
  ResponseHandler.prototype.YAML_COMMANDS = {
    'stats-tube': true,
    'stats': true,
    'stats-job': true
  };
  ...
  ResponseHandler.prototype.parse_body = function() {
    ...
    if ((typeof (_a = this.body) !== "undefined" && _a !== null)) {
      ...
      if (this.body.length === (body_length + 2)) {
        ...
        var body = this.body.substr(0, this.body.length - 2);
        if (this.YAML_COMMANDS[this.command_name]) {
            try {
